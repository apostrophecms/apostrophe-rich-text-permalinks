        ...
        return self.apos.docs.find(req, {
          ...
          _.each(docs, function(doc) {
            ...
            _.each(widgets, function(widget) {
              ...
              while (true) {
                i = widget.content.indexOf('apostrophe-permalink-' + doc._id, offset);
                if (i === -1) {
                  break;
                }
                offset = i + ('apostrophe-permalink-' + doc._id).length;
                var updateTitle = widget.content.indexOf('?updateTitle=1', i);
                ...
                var left = widget.content.lastIndexOf('<', i);
                var href = widget.content.indexOf(' href="', left);
                var close = widget.content.indexOf('"', href + 7);
                if (!widget._edit) {
                  if ((left !== -1) && (href !== -1) && (close !== -1)) {
                    widget.content = widget.content.substr(0, href + 6) + doc._url + widget.content.substr(close + 1);
                  } else {
                ...
                var right = widget.content.indexOf('>', left);
                var nextLeft = widget.content.indexOf('<', right);
                if ((right !== -1) && (nextLeft !== -1)) {
                  widget.content = widget.content.substr(0, right + 1) + self.apos.utils.escapeHtml(doc.title) + widget.content.substr(nextLeft);
                }
              }
            });
          });
